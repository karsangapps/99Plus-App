...
export default function SelectionHubError({ error, reset }: { error: Error; reset: () => void }) {
  return (
    <div className="flex-1 flex items-center justify-center min-h-screen" style={{ background: '#F8FAFC' }}>
      <div className="text-center p-8 max-w-sm">
        ...
        <h2 className="text-lg font-bold mb-1" style={{ color: '#0F172A' }}>Selection Hub failed to load</h2>
        <p className="text-sm mb-5" style={{ color: '#9CA3AF' }}>{error.message || 'Could not load cutoff data.'}</p>
        <button onClick={reset}
          className="px-5 py-2.5 rounded-xl text-sm font-bold text-white"
          style={{ background: '#6366F1' }}
          aria-label="Retry loading the Selection Hub">
          Try Again
        </button>
      </div>
    </div>
