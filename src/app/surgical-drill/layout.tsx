...
const navItems = [
  ...
  { href: '/diagnosis',       icon: 'fa-stethoscope',      label: 'Diagnosis' },
  { href: '/surgical-drill',  icon: 'fa-crosshairs',       label: 'Surgical Drill', active: true },
  { href: '/selection-hub',   icon: 'fa-building-columns', label: 'Selection Hub' },
]

export default async function SurgicalDrillLayout({ children }: { children: React.ReactNode }) {
      ...
      <header className="lg:hidden sticky top-0 z-40 bg-[#0F172A] text-white px-4 py-3 flex items-center justify-between shadow-md">
        <div className="flex items-center gap-3">
          <div className="w-7 h-7 rounded-lg bg-indigo-500/15 flex items-center justify-center">
            <i className="fa-solid fa-crosshairs text-indigo-400 text-sm" />
          </div>
          <span className="text-sm font-bold">99Plus · Surgical Drill</span>
        </div>
